...
const Cards = ({ task, projectId }: { task: Task, projectId: string }) => {
    ...
    return (
        <div className={`${taskUpdate.status == TaskStatus.ToDo ? "border-l-[#23B7E5]/40" : taskUpdate.status == TaskStatus.Doing ? "border-l-[#845ADF]/40" : "border-l-[#26BF94]/40"} flex justify-between p-2 w-full bg-[#1A1C1E] h-fit border-l-[7px] rounded-lg ${isDragging && 'opacity-75'}`} ref={setNodeRef}
            {...attributes}
            style={{
                transition,
            ...
            <div className='flex flex-col gap-y-5'>
                <div className='flex gap-x-2 items-center'>
                    ...
                    <h1 className='font-semibold text-xs font-Poppins text-white/70'  {...listeners}>{taskUpdate.taskName}</h1>
                </div>
                <div>
                    <ul className='flex flex-col gap-y-3 font-normal text-xs font-Poppins text-white/70'>
                        <li>Assigned On : <span className='text-[12px] text-white/50'>{new Date(taskUpdate.created_at).toLocaleDateString()}</span></li>
                        <li>Target Date : <span className='text-[12px] text-white/50'>{new Date(taskUpdate.deadline).toLocaleDateString()}</span></li>
                        <li className='relative flex items-center gap-x-1'>
                            ...
                            <div className='flex gap-x-1 items-center' onClick={() => setSearchMembers(!searchMembers)}>
                                ...
                                {taskUpdate.assignedTo && <AnimatedTooltip items={taskUpdate.assignedTo} />}
                    ...
                    {showPriority &&
                        <div className='flex gap-x-5 absolute top-7 bg-slate-800 rounded-md p-3 z-10'>
                            <button className={cn("bg-[#E6533C]/10 hover:bg-[#E6533C]/40 text-[#E6533C]  w-fit py-1 px-2 text-center font-semibold rounded-[4px] text-[11px] duration-300 capitalize",
                                TaskPriority.High === taskUpdate.priority && "hidden"
                            ...
                            <button className={cn("bg-[#F5B849]/10 hover:bg-[#F5B849]/40 text-[#F5B849] w-fit py-1 px-2 text-center font-semibold rounded-[4px] text-[11px] duration-300 capitalize",
                                TaskPriority.Medium === taskUpdate.priority && "hidden"
                            ...
                            <button className={cn("bg-[#26BF94]/10 hover:bg-[#26BF94]/40 text-[#26BF94]  w-fit py-1 px-2 text-center font-semibold rounded-[4px] text-[11px] duration-300 capitalize",
                                TaskPriority.Low === taskUpdate.priority && "hidden"
