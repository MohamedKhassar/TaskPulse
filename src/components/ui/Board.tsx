                ...
                <div className='overflow-y-scroll flex flex-col gap-y-5 h-fit max-h-80 min-h-fit lg:min-w-64 min-w-56 scroll p-3'>
                    ...
                    <form action="" className='my-2 flex flex-col gap-y-3 items-end' onSubmit={handleAddTask}>
                        ...
                        <div className="relative bg-inherit">
                            <input value={taskData.taskName} onChange={(e) => setTaskData({ ...taskData, taskName: e.target.value, status: column })} type="text" id={column} name="username" className="peer bg-transparent p-2 w-full rounded-lg text-gray-200 placeholder-transparent ring-2 ring-gray-500 focus:ring-sky-600 focus:outline-none focus:border-rose-600" placeholder="Type inside me" /><label htmlFor={column} className="absolute cursor-text left-0 -top-3 text-sm text-gray-500 bg-inherit mx-1 px-1 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-placeholder-shown:top-2 peer-focus:-top-3 peer-focus:text-sky-600 peer-focus:text-sm transition-all peer-focus:bg-[#0B0C0C]">Type inside me</label>
                        </div>
